cube.piece.type.sum = function(board) {
  ...
  if (mode == "edit") {
    ...
    time.append("select").selectAll("option")
        .data([
          ...
          {description: "Minutes (1)", value: 6e4},
  ...
  function deserialize(json) {
    ...
    if (mode == "edit") {
      ...
      //socket = new WebSocket("ws://" + location.hostname + ":1081" + "/1.0/metric/get");
      socket = new WebSocket("ws://" + cube['evaluator-http-host'] + ":" + cube['evaluator-http-port'] + "/1.0/metric/get");
      socket.onopen = load;
      ...
      function store(message) {
        var tmp = JSON.parse(message.data).value;
