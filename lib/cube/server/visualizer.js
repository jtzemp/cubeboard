...
exports.register = function(db, endpoints, options) {
  var config_file = resolve("cube-config.js");
  var config_data = "\n// auto-generated from cubeboard-config.js \n" +
    "cube['evaluator-http-host'] = '" + options['evaluator-http-host'] + "';\n" +
    "cube['evaluator-http-port'] = " + options['evaluator-http-port'] + ";\n" +
    "\n";
  fs.writeFileSync(config_file, config_data, 'utf8');
  console.log("cube-config.js: " + resolve("cube-config.js"));
  ...
  endpoints.http.push(
    ...
    endpoint.exact("/jquery.min.js", endpoint.file(
      path.join(__dirname, "..", "jquery-1.8.3.min.js")
    )),
    endpoint.exact("/jquery-ui.custom.min.js", endpoint.file(
      path.join(__dirname, "..", "jquery-ui-1.9.2.custom.min.js")
    )),
    endpoint.exact("/cube.js", endpoint.file(
      resolve("start.js"),
      resolve("cube.js"),
      resolve("cube-config.js"),
      resolve("piece.js"),
      resolve("piece-area.js"),
      resolve("piece-sum.js"),
      resolve("piece-text.js"),
      resolve("piece-html.js"),
      resolve("piece-script.js"),
      resolve("palette.js"),
      resolve("squares.js"),
      resolve("board.js"),
      resolve("header.js"),
...
function createBoard(db) {
  ...
  return function random(request, response) {
    ...
    var path = id.toString(36);
    if (path.length < 6) path = new Array(7 - path.length).join("0") + path;

    boards.insert({_id: id, "path": path, "name": ""}, {safe: true}, function(error) {
      if (error) {
        if (/^E11000/.test(error.message)) return random(request, response); // duplicate
        response.writeHead(500, {"Content-Type": "text/plain"});
        response.end("500 Server Error");
      } else {
        response.writeHead(302, {"Location": "http://" + request.headers["host"] + "/" + path + "/edit"});
        response.end();
      }
    });
  };
